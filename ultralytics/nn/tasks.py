...
from ultralytics.nn.extra_modules import TransModule
from ultralytics.nn.extra_modules import OriModule
from ultralytics.nn.extra_modules import EndogenousFeatureDecoupling
from ultralytics.nn.extra_modules.domain_classifier import DomainClassifier
from ultralytics.nn.extra_modules.block import IBN, BottleNeck_IBN, ConvNormLayer_IBN, SW, BottleNeck_SW, ConvNormLayer_SW, IterNorm, BottleNeck_IterNorm, ConvNormLayer_IterNorm, NormalizationPerturbation, BottleNeck_NP, ConvNormLayer_NP
...
class RTDETRDetectionModel(DetectionModel):
    ...
    def init_criterion(self):
        ...
        return RTDETRDetectionLoss(nc=self.nc,
                                   ...
                                   use_emasvfl=False,
                                   use_mal=False,
                                   domain_loss_weight=self.yaml.get('domain_loss_weight', 1.5))
    ...
    def loss(self, batch, preds=None):
        ...
        if domain_labels is not None:
            domain_labels = domain_labels.to(device=img.device, dtype=torch.long)

        preds = self.predict(img,
                             batch=targets,
                             domain_epoch=batch.get('domain_epoch', 0)) if preds is None else preds
        domain_logits = None
        if self.training and isinstance(preds, tuple) and len(preds) == 2:
            head_output, domain_logits = preds
        else:
            head_output = preds

        dec_bboxes, dec_scores, enc_bboxes, enc_scores, dn_meta = \
            head_output if self.training else head_output[1]
        if dn_meta is None:
        ...
        # Backpropagate all RT-DETR losses and expose the three detection losses plus the CIM domain loss.
        return sum(loss.values()), torch.as_tensor([loss[k].detach() for k in
                                                    ['loss_giou', 'loss_class', 'loss_bbox', 'loss_domain']],
                                                   device=img.device)

    def predict(self, x, profile=False, visualize=False, batch=None, augment=False, domain_epoch=0):
        """
        ...
        for m in self.model[:-1]:  # except the head part
            ...
            if hasattr(m, 'backbone'):
                ...
                if isinstance(m, DomainClassifier):
                    # CIM is needed only during training. Preserve the P5 feature
                    # for the detection branch while retaining domain logits for loss.
                    if self.training:
                        domain_logits = m(x, epoch=domain_epoch)
                else:
                    x = m(x)  # run
                y.append(x if m.i in self.save else None)  # save output
            if visualize:
...
def parse_model(d, ch, verbose=True, warehouse_manager=None):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in (Classify, Conv, ConvTranspose, GhostConv, Bottleneck, GhostBottleneck, SPP, SPPF, DWConv, DSConv, Focus,
                 ...
                 C2f_PSFSConv, C2f_FasterSFSConv, C2f_GroupMamba, C2f_GroupMambaBlock, C2f_MambaVision, BottleNeck_IBN, SW, BottleNeck_SW, ConvNormLayer_SW, IterNorm, BottleNeck_IterNorm, ConvNormLayer_IterNorm, NormalizationPerturbation, BottleNeck_NP, ConvNormLayer_NP):
            if args[0] == 'head_channel':
                args[0] = d[args[0]]
            c1, c2 = ch[f], args[0]
            ...
            c2 = ch[f]
            args = []  # OriModule不需要参数
        elif m is TransModule:
            c2 = ch[f]
            # TransModule需要参数: c, endogenous_prob, aging_level
            if len(args) >= 3:
                args = [args[0], args[1], args[2]]  # [c, endogenous_prob, aging_level]
            elif len(args) == 2:
                args = [args[0], args[1], 0.5]  # [c, endogenous_prob, default_aging_level]
            else:
                args = [3, 1.0, 0.5]  # 默认参数
        elif m is EndogenousFeatureDecoupling:
            c2 = ch[f]
            # 使用前一层的输出通道数，但使用YAML中配置的reduction参数
            if len(args) >= 2:
                args = [c2, args[1], args[2] if len(args) >= 3 else 0.5]  # [channels=c2, reduction, decoupling_strength]
            else:
                args = [c2, 32, 0.5]  # 默认参数: channels=c2, reduction=32, decoupling_strength=0.5
        elif m in {Concat}:
            ...
            c1 = ch[f]
            hidden_dim = args[0]
            num_domains = args[1] if len(args) > 1 else 2
            c2 = num_domains
            args = [c1, hidden_dim, num_domains]
        elif m is Fusion:
            args[0] = d[args[0]]
