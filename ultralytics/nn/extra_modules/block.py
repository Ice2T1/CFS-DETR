...
__all__ = ['Ghost_HGBlock', 'Rep_HGBlock', 'DWRC3', 'C3_DWR', 'C2f_DWR', 'BasicBlock_Ortho', 'BottleNeck_Ortho',
           ...
           'C2f_SAVSS', 'C2f_MambaOut', 'IBN', 'BottleNeck_IBN', 'ConvNormLayer_IBN', 'SW', 'BottleNeck_SW', 'ConvNormLayer_SW', 'IterNorm', 'BottleNeck_IterNorm', 'ConvNormLayer_IterNorm', 'NormalizationPerturbation', 'BottleNeck_NP', 'ConvNormLayer_NP'
           ]
...
# Public names used in the CFS-DETR paper.
UniversalEdgeEnhancer = EdgeEnhancer
FiberSEMEdgeEnhancer = MutilScaleEdgeInformationSelect
FSENetBlock = CSP_MutilScaleEdgeInformationSelect
...
######################################## IBN (Instance-Batch Normalization) start ########################################

class IBN(nn.Module):
    """Instance-Batch Normalization layer from 'Two at Once: Enhancing Learning and Generalization Capacities via IBN-Net'"""
    ...
    def forward(self, x):
        split = torch.split(x, [self.half, x.size(1) - self.half], 1)
...
class BottleNeck_IBN(BottleNeck):
    """Bottleneck residual block with IBN-a (Instance-Batch Normalization)
    默认开启IBN-a，可以直接在YAML中替换BottleNeck使用
    """
    ...
    def __init__(self, ch_in, ch_out, stride, shortcut, act='relu', variant='d'):
        ...
        # 重新定义第一个卷积层，使用IBN
        self.branch2a = ConvNormLayer_IBN(ch_in, width, 1, stride1, act=act)
...
class ConvNormLayer_IBN(nn.Module):
    """ConvNormLayer with IBN (默认使用IBN)"""
...
class SW(nn.Module):
    """Switchable Whitening layer - 简化版本，像IBN一样易用"""
    ...
    def __init__(self, planes):
        ...
        num_pergroup = 16
        # 自动调整分组大小
        if planes % num_pergroup != 0:
            for divisor in [8, 4, 2, 1]:
                if planes % divisor == 0:
                    num_pergroup = divisor
                    break
        
        self.num_features = planes
        self.num_pergroup = num_pergroup
        self.num_groups = planes // num_pergroup
        self.sw_type = sw_type
        self.eps = 1e-5
        self.momentum = 0.9
        
        # 可学习权重
        self.sw_mean_weight = nn.Parameter(torch.ones(sw_type))
        self.sw_var_weight = nn.Parameter(torch.ones(sw_type))
        
        # 仿射变换
        self.weight = nn.Parameter(torch.ones(planes))
    ...
    def forward(self, x):
        ...
        eye = torch.eye(c, device=x.device, dtype=x.dtype).view(1, c, c).expand(N * g, c, c)
        cov = var_weight[0] * cov_bn + var_weight[1] * cov_in + self.eps * eye
        
        # 简化的白化 (使用Cholesky分解替代Newton迭代)
        try:
            L = torch.linalg.cholesky(cov)
            wm = torch.linalg.solve_triangular(L, eye, upper=False)
            wm = torch.linalg.solve_triangular(L.transpose(-2, -1), wm, upper=True)
        except:
            # 退化为标准化
            std = torch.sqrt(torch.diagonal(cov, dim1=-2, dim2=-1) + self.eps).unsqueeze(-1)
            wm = torch.diag_embed(1.0 / std.squeeze(-1))
...
class BottleNeck_SW(BottleNeck):
    """Bottleneck residual block with SW (Switchable Whitening)
    默认开启SW，可以直接在YAML中替换BottleNeck使用
    """
    ...
    def __init__(self, ch_in, ch_out, stride, shortcut, act='relu', variant='d'):
        ...
        # 重新定义第二个卷积层，使用SW
        self.branch2b = ConvNormLayer_SW(width, width, 3, stride2, act=act)
...
class ConvNormLayer_SW(nn.Module):
    """ConvNormLayer with SW (默认使用SW)"""
...
class IterNorm(nn.Module):
    """Iterative Normalization layer - 迭代归一化，像IBN一样易用
    
    基于论文: Iterative Normalization: Beyond Standardization towards Efficient Whitening (CVPR 2019)
    核心思想：使用Power Iteration方法迭代计算协方差矩阵的逆平方根，实现高效白化
    完全模仿SW的实现模式，确保deepcopy兼容性
    """
    ...
    def __init__(self, planes):
        ...
        self.num_features = planes
        self.num_pergroup = num_pergroup  # 使用与SW相同的变量名
        self.num_groups = planes // num_pergroup  # 使用与SW相同的计算方式
        ...
        # 运行时统计 - 完全模仿SW的buffer注册方式
        self.register_buffer('running_mean', torch.zeros(self.num_groups, num_pergroup, 1))
        self.register_buffer('running_wm', torch.eye(num_pergroup).unsqueeze(0).repeat(self.num_groups, 1, 1))
    
    def forward(self, x):
        N, C, H, W = x.size()
        c, g = self.num_pergroup, self.num_groups  # 使用与SW相同的变量名
        ...
        if self.training:
            ...
            # Power Iteration for whitening matrix - IterNorm的核心
            eye = torch.eye(c, device=x.device, dtype=x.dtype).unsqueeze(0).expand(g, -1, -1)
            trace = torch.diagonal(cov_bn, dim1=-2, dim2=-1).sum(-1, keepdim=True).unsqueeze(-1)
            cov_norm = cov_bn / (trace + self.eps)
            
            # 初始化迭代矩阵
            X = eye.clone()
            
            # Power Iteration: X = (3*X - X*X*X*cov_norm) / 2
            for _ in range(self.num_iterations):
                X3 = torch.bmm(torch.bmm(X, X), X)
                X3_cov = torch.bmm(X3, cov_norm)
                X = (3.0 * X - X3_cov) / 2.0
            
            # 最终的白化矩阵
            wm = X / torch.sqrt(trace + self.eps)
            
            # 更新运行时统计量 - 模仿SW的更新方式
            self.running_mean.mul_(self.momentum).add_(mean_bn, alpha=1 - self.momentum)
            self.running_wm.mul_(self.momentum).add_(wm, alpha=1 - self.momentum)
            
            # 应用白化
            out_data = torch.bmm(wm, in_data_bn)
        else:
            # 推理模式：使用运行时统计量
            in_data_bn = in_data_t - self.running_mean
            out_data = torch.bmm(self.running_wm, in_data_bn)
        
        # 重组输出 - 模仿SW的方式
        out_data = out_data.view(C, N, H, W).transpose(0, 1).contiguous()
        
        # 仿射变换
        out_data = out_data * self.weight.view(1, -1, 1, 1) + self.bias.view(1, -1, 1, 1)
        
        return out_data


class BottleNeck_IterNorm(BottleNeck):
    """Bottleneck residual block with IterNorm (Iterative Normalization)
    默认开启IterNorm，可以直接在YAML中替换BottleNeck使用
    """
    
    def __init__(self, ch_in, ch_out, stride, shortcut, act='relu', variant='d'):
        super().__init__(ch_in, ch_out, stride, shortcut, act, variant)
        
        # 替换第二个卷积层的BN为IterNorm (关键层)
        width = ch_out
        if variant == 'a':
            stride1, stride2 = stride, 1
        else:
        ...
        # 重新定义第二个卷积层，使用IterNorm
        self.branch2b = ConvNormLayer_IterNorm(width, width, 3, stride2, act=act)
...
class ConvNormLayer_IterNorm(nn.Module):
    """ConvNormLayer with IterNorm (默认使用IterNorm)"""
    ...
    def __init__(self, ch_in, ch_out, kernel_size, stride, padding=None, bias=False, act=None):
        ...
        # 使用IterNorm替代普通的BatchNorm
        self.norm = IterNorm(ch_out)
        self.act = nn.Identity() if act is None else get_activation(act) 
...
class NormalizationPerturbation(nn.Module):
    """Normalization Perturbation for robust object detection
    
    基于论文: Towards robust object detection invariant to real-world domain shifts (ICLR 2023)
    核心思想：通过随机扰动特征的归一化统计量来增强模型对域偏移的鲁棒性
    """
    
    def __init__(self):
        super(NormalizationPerturbation, self).__init__()
        self.std_scale = 0.75
        self.apply_prob = 0.5
        self.use_plus = False
    ...
    def forward(self, feat):
        ...
        Returns:
            output: perturbed features of size (B, C, H, W)
        """
        if not self.training:
            return feat
        ...
        # 随机决定是否应用扰动
        p = torch.rand(1).item()
        if p >= self.apply_prob:
            return feat
        
        if self.use_plus:
            return self._normalization_perturbation_plus(feat)
        else:
            return self._normalization_perturbation(feat)
    
    def _normalization_perturbation(self, feat):
        """基础版NP"""
        # feat: (B, C, H, W)
        feat_mean = feat.mean((2, 3), keepdim=True)  # (B, C, 1, 1)
        ones_mat = torch.ones_like(feat_mean)
        
        # 生成随机扰动参数
        alpha = torch.normal(ones_mat, self.std_scale * ones_mat)  # (B, C, 1, 1)
        beta = torch.normal(ones_mat, self.std_scale * ones_mat)   # (B, C, 1, 1)
        
        # 应用扰动: alpha * feat - alpha * feat_mean + beta * feat_mean
        output = alpha * feat - alpha * feat_mean + beta * feat_mean
        return output
    
    def _normalization_perturbation_plus(self, feat):
        """增强版NP+"""
        feat_mean = feat.mean((2, 3), keepdim=True)  # (B, C, 1, 1)
        ones_mat = torch.ones_like(feat_mean)
        ...
        # 计算自适应缩放因子
        mean_diff = torch.std(feat_mean, dim=0, keepdim=True)  # (1, C, 1, 1)
        mean_scale = mean_diff / (mean_diff.max() + 1e-8) * 1.5
        
        # 生成随机扰动参数
        alpha = torch.normal(ones_mat, self.std_scale * ones_mat)
        beta = 1 + torch.normal(zeros_mat, self.std_scale * ones_mat) * mean_scale
...
class BottleNeck_NP(BottleNeck):
    """Bottleneck residual block with Normalization Perturbation
    增强对域偏移的鲁棒性，可以直接在YAML中替换BottleNeck使用
    """
    ...
    def __init__(self, ch_in, ch_out, stride, shortcut, act='relu', variant='d'):
        ...
        # 添加NP模块，通常在第二个卷积后应用
        self.np = NormalizationPerturbation()
...
class ConvNormLayer_NP(nn.Module):
    """ConvNormLayer with Normalization Perturbation"""
    ...
    def forward(self, x):
        x = self.conv(x)
        x = self.norm(x)
        x = self.np(x)  # 在归一化后应用NP
        return self.act(x)




######################################## IBN (Instance-Batch Normalization) end ########################################
