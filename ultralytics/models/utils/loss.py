...
class RTDETRDetectionLoss(DETRLoss):
    ...
    def __init__(self, *args, domain_loss_weight=1.5, **kwargs):
        """Initialize RT-DETR losses and the CIM domain-loss weight."""
    ...
    def forward(self, preds, batch, dn_bboxes=None, dn_scores=None, dn_meta=None,
        ...
        """
        # 不处理preds，直接使用传入的preds作为检测分支的输入
        ...
        # The GRL in DomainClassifier makes this ordinary cross-entropy loss
        # adversarial with respect to the feature extractor.
        if domain_logits is not None and domain_labels is not None:
            domain_labels = domain_labels.to(device=domain_logits.device, dtype=torch.long).view(-1)
            if domain_logits.shape[0] != domain_labels.shape[0]:
                raise ValueError(
                    f'Domain prediction batch size ({domain_logits.shape[0]}) does not match '
                    f'domain label batch size ({domain_labels.shape[0]}).')
            domain_loss = F.cross_entropy(domain_logits, domain_labels)
            total_loss['loss_domain'] = self.domain_loss_weight * domain_loss
        else:
            total_loss['loss_domain'] = torch.tensor(0., device=self.device)
