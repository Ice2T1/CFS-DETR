...
class HLPAttention(nn.Module):
    ...
    def _get_pos_2d(self, h, w): 
        xi, yi = torch.meshgrid(torch.arange(0.9, 1.1, 0.001), torch.arange(0.9, 1.1, 0.001))
        sh = (201-h)//2
        sw = (201-w)//2
        index_h = xi[sh:sh+h, sw:sw+w].reshape(1, 1, 1, h, w).cuda()
        index_w = yi[sh:sh+h, sw:sw+w].reshape(1, 1, 1, h, w).cuda()
        pos_x = torch.exp( -self.alpha * index_h**2 ) * torch.exp(2 * self.alpha * torch.log(index_h+1.0)) 
        pos_y = torch.exp( -self.alpha * index_w**2 ) * torch.exp(2 * self.alpha * torch.log(index_w+1.0)) 
        pos_emb = pos_x * pos_y
        return pos_emb.reshape(1, 1, 1, -1)
    def forward(self, x, H, W): #
        B, N, C = x.shape
        x = x.reshape(B, N, self.heads, C // self.heads).permute(0, 2, 1, 3)
        qkv = self.qkv_proj(x).permute(0,1,3,2) #b heads c' h*w
        q, k, v = torch.split(qkv, split_size_or_sections=[self.embed_dim, self.embed_dim, self.embed_dim], dim=2)
        pos_emb = self._get_pos_2d(H, W)
        out = self.attn_dropout((q*pos_emb) @ (k).permute(0, 1, 3, 2)) / (N*C//self.heads)
        out = (out @ (v* pos_emb)).permute(0,1,3,2)
        out = self.out_proj(out).permute(0,2,1,3).reshape(B, N, C) 
        return out
